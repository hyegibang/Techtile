...
class CharMenuDisplay(object):
	...
	def __init__(self, adafruit_char_lcd_plate, nodes, inputlist):
		if nodes is None:
			nodes = []
			...
			try:
				if self.lcd.is_pressed(LCD.DOWN):
					if(self.highlighted_index < len(self.context_node.nodes) - 1):
						...
						self._draw()
						print(self.highlighted_index)
						
						
					else: # when on the end of list 
						self._blink() 
				...
				if self.lcd.is_pressed(LCD.RIGHT): # select
					...
					if len(self.highlighted_node.nodes) > 0:
						...
						self._draw()
                                                ...
                                                #selected = self.inputlist[self.highlighted_index]
                                                #print(selected)
                                                #selected = str(float(self.highlighted_index))
                                                selected = self.highlighted_index
                                                #selected = self.inputlist[self.highlighted_index]
                                                #print("classfunction", selected)
                                                return selected

				if self.lcd.is_pressed(LCD.LEFT): # read
					...
					if self.context_node.parent is not None:
						...
						self._draw()
                                                ...
                                                #print(self.root_node.nodes[self.highlighted_index])
                                                #print(self.inputlist)
                                                print(self.inputlist[self.highlighted_index])
                                                engine.say(self.inputlist[self.highlighted_index])
						engine.runAndWait()
